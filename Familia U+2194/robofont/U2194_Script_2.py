...
for behavior in behaviors:
    ...
    for weight in weigths:
        ...
        font_dest.info.styleName = "%s" % (behavior)
        font_dest.info.fullName = "%s %s" % (family_name, behavior)
                ...
                for points in work_glyph[contour].points:
                    ...
                    if str(points.type) is not "offCurve":
                        if randint(0, 10) < 8:
                            ...
                            work_glyph_contour_point_list += [(points.x+randint(0,200), points.y)]
